...
- **Cloud onboarding leg = Gemini via Firebase AI Logic, `firebase-ai` SDK (maintainer-funded).** Binding resolved at wire time (2026-08-16, see `docs/edgelab/plan.md`): the `firebase-ai` SDK, not a hand-written Functions proxy — AI Logic already keeps the key off the client, so there is no server code to write, deploy, or fund. Rejected keyless/free-tier alone (function-calling free tiers largely need *someone's* key) and HF Inference (weakest function-calling quality — bad onboarding demo). Default model `gemini-3.5-flash-lite` (price/quality tradeoff; rationale in `docs/edgelab/research-cloud-models-interactions-api.md`). Live and verified on device (2026-08-27): App Check installed (debug token → registered, release → Play Integrity), config stays as shipped. BYOK is a later iteration. **Out of scope for v1: the Interactions API** (Gemini-API-only surface; adopting it would put a key client-side or require a backend proxy, contradicting this decision).
...
- ~~Playground UX missing~~ — **built** (2026-08): Playground screen with preset-library authoring, annotated Trace with `[used/ignored tool output]` tags.
- ~~Cloud onboarding backend not built~~ — **live** (2026-08): Firebase AI Logic via the `firebase-ai` SDK, App Check enforced, first real Gemini turn verified on device.
